...
void angleSort(iterator begin, iterator end) {
...
template <class iterator>
float area(iterator begin, iterator end) {
  float area = 0;
...
template <class iterator>
bool pointInPolygon(iterator begin, iterator end, const Vector2d &p) {
...
void correctWinding(iterator begin, iterator end) {
  if (area(begin, end) > 0) std::reverse(begin, end);
}
}
